...
require('../src/roles.js');           // sets window.PARTY_BASE + window.PARTY_GAME
reqCore('ui/sound.js');
require('../src/ui-glue.js');
reqCore('ui/ui-core.js');

var DAYBREAK = require('../src/roles.js');
var ROLE_NAMES = DAYBREAK.roles.map(function (r) { return r.name; });
...
function assertNoLeak(where) {
  ...
  ROLE_NAMES.forEach(function (rn) {
    ok(h.indexOf('>' + rn) === -1 && h.indexOf(rn + '<') === -1 && h.indexOf(' ' + rn + ' ') === -1, 'no role-name leak ("' + rn + '") on shared screen: ' + where);
  });
}
...
function autoAct() {
  for (var i = 0; i < 16; i++) {
    ...
    if (centers.length) {
      ...
      var unsel = centers.filter(function (x) { return !/ sel/.test(' ' + x._class); });
      var sel = centers.length - unsel.length;
      if (sel < need && unsel.length) { unsel[0].click(); continue; }
    }
    break;
  }
...
function playOneGame(label, botClicks) {
  ...
  clickAct('start');
  ok(/Pass the phone to/.test(html()), label + ': reached first reveal gate');
  var guard = 0, sawGate = false, sawDay = false, sawVoteGate = false;
  while (guard++ < 900) {
    var h = html();
    ...
    else if (byAct('to-vote')) { sawDay = true; clickAct('to-vote'); }            // DAY exempt (Revealer)
    ...
    else { ok(false, label + ': UI stuck at ' + h.slice(0, 160)); break; }
  }
  ok(/result-win/.test(html()), label + ': reached an end/result screen');
  ok(sawGate && sawDay && sawVoteGate, label + ': exercised reveal, day, and vote screens');
  clickAct('home');
}
