/*
 * roles.js (Werewolf) — the role registry + presets for the Werewolf game.
 *
 * Pure data + small act()/inputs() handlers that drive the shared engine. No DOM.
 * All narration/blurb text is original wording. UMD: Node tests require() this; the
 * inlined browser bundle reads window.WEREWOLF_GAME.
 *
 * Wake order (lower acts first; null = never wakes at night):
...
(function (root, factory) {
  ...
  if (root) { root.PARTY_GAME = G; root.WEREWOLF_GAME = G; } // PARTY_GAME = generic handle for the shared UI
})(typeof window !== 'undefined' ? window : (typeof globalThis !== 'undefined' ? globalThis : this), function () {
  'use strict';

  // helper: count seats currently acting as a werewolf (dealt wolf or doppelgänger-copied wolf)
  ...
  var roles = [
    {
      id: 'werewolf', name: 'Werewolf', team: 'werewolf', werewolf: true, wake: 2,
      maxCopies: 2,
      ...
      act: function (ctx, inputs) {
        ...
        if (inputs.peek === 'yes' && inputs.center && inputs.center.length) {
...

    {
      ...
      narration: {
        open: 'Minion, wake up. Werewolves, raise a thumb so the Minion can see you.',
        close: 'Werewolves, lower your thumbs. Minion, close your eyes.'
      },
...

    {
      ...
      narration: {
        open: 'Masons, wake up and look for the other Mason.',
        close: 'Masons, close your eyes.'
      },
      inputs: function () { return []; },
      act: function (ctx) { ctx.identifyAllies(function (s) { return ctx.actingRole(s) === 'mason'; }); },
      validate: function (q) {
        if (q.counts.mason === 1) return [{ level: 'error', text: 'Masons come as a pair — use 0 or 2.' }];
        return [];
      }
    },

    {
      ...
      narration: {
        open: 'Seer, wake up. You may look at one other player\'s card, or two of the center cards.',
        close: 'Seer, close your eyes.'
      },
      inputs: function (ctx) {
        return [
          { id: 'mode', type: 'choice', label: 'How will you look?', options: [
            { value: 'player', label: 'One player\'s card' },
            { value: 'center', label: 'Two center cards' },
            { value: 'skip', label: 'Do not look' }
          ] },
...

    {
      ...
      blurb: 'You may swap your card with another player\'s, then look at your new card. You become that role (you do not perform its action).',
      prompt: 'You may swap your card with another player\'s, then see your new card.',
      narration: {
        open: 'Robber, wake up. You may swap your card with another player\'s, then look at your new card.',
        close: 'Robber, close your eyes.'
      },
      inputs: function (ctx) {
        return [{ id: 'target', type: 'pickPlayer', label: 'Rob whose card? (or skip)', exclude: [ctx.self], optional: true }];
      },
      act: function (ctx, inputs) {
        if (inputs.target == null) { ctx.noop('Robber chose not to rob.'); return; }
        ctx.swap(ctx.seatPos(ctx.self), ctx.seatPos(inputs.target));
        ctx.lookCard(ctx.seatPos(ctx.self)); // see the card you just took
      }
    },

    {
      ...
      narration: {
        open: 'Troublemaker, wake up. You may swap the cards of two other players.',
        close: 'Troublemaker, close your eyes.'
      },
      inputs: function (ctx) {
        return [
...

    {
      ...
      narration: {
        open: 'Drunk, wake up and swap your card with a card from the center.',
        close: 'Drunk, close your eyes.'
      },
      inputs: function () {
        return [{ id: 'center', type: 'pickCenter', count: 1, label: 'Take which center card? (you will not see it)' }];
      },
      act: function (ctx, inputs) {
        ctx.swap(ctx.seatPos(ctx.self), ctx.centerPos(inputs.center[0]));
        ctx.noop('You took a center card without looking.');
      }
    },

    {
      ...
      narration: {
        open: 'Insomniac, wake up and look at your own card.',
        close: 'Insomniac, close your eyes.'
      },
      inputs: function () { return []; },
      act: function (ctx) { ctx.lookCard(ctx.seatPos(ctx.self)); },
      validate: function (q) {
        var swapper = q.has('robber') || q.has('troublemaker') || q.has('drunk');
        if (!swapper) return [{ level: 'warn', text: 'Insomniac has little to do without a Robber, Troublemaker, or Drunk in play.' }];
        return [];
      }
    },

    {
      ...
      // No night action. Vote-time modifier: a dying Hunter takes their vote target with them.
...

    {
      ...
      // Win handled by base resolution (tanner flag).
    },

    {
      id: 'villager', name: 'Villager', team: 'village', wake: null,
      blurb: 'No special ability — but definitely not a Werewolf.'
    },

    {
      id: 'doppelganger', name: 'Doppelgänger', team: function (state, seat) {
        // team travels with whatever the doppelgänger copied (resolved via copiedRole on the card)
        return 'village';
      }, wake: -7, maxCopies: 1,
      ...
      narration: {
        open: 'Doppelgänger, wake up and look at one other player\'s card. You are now that role.',
        close: 'Doppelgänger, close your eyes.'
      },
      inputs: function (ctx) {
        return [{ id: 'target', type: 'pickPlayer', label: 'Copy whose card?', exclude: [ctx.self] }];
      },
      act: function (ctx, inputs) {
      ...
      validate: function () {
        return [{ level: 'warn', text: 'The Doppelgänger adds the most complexity — best added once the table knows the other roles.' }];
      }
    }
  ];

  // Recommended presets per player count (cards = players + 3). Tuned for fun, faithful balance.
  // Each list length === playerCount + 3.
  var presets = {
